...
if __name__ == "__main__":
    ...
    for n in tqdm([6, 10]):
        ...
        for p in range(1,2):
            ...
            Result = []
            for bbeta in np.linspace(-0.5, 0.5, 101):
                bbeta = np.round(bbeta, 3)
                print("\n n"+str(n)+"beta"+str(bbeta))
                for ggamma in np.linspace(-1.0, 1.0, 201):
                    ggamma - np.round(ggamma, 3)
                    init_params = list(np.concatenate(( [bbeta] , [ggamma])))
                    counts = Qiskit_QAOA( init_params[:p], init_params[p:], V, E)
                    avr_C       = 0
                    for sample in list(counts.keys()):
                        # use sampled bit string x to compute C(x)
                        x         = [int(num) for num in list(sample)]
            ...
            filename = "./grid/grid"+str(n)+"nodes_p" +str(p)
