...
ggamma = np.linspace(-1*np.pi, np.pi, 200)
bbeta = np.linspace(0*np.pi, 0.5*np.pi, 50)
...
plt.contourf(ggamma, bbeta, val2[50:])
...
plt.yticks([0, np.pi/8, np.pi/4, 3*np.pi/8, np.pi/2], 
           [r"$0$", r"$0.125\pi$", r"$0.250\pi$", r"$0.375\pi$", r"$0.500\pi$"])
plt.ylabel(r"$\beta$")
plt.colorbar()
#plt.title("N = 6, P=2")
plt.show()
